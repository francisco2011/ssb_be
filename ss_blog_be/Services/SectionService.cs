...
namespace ss_blog_be.Services
{
    public class SectionService
    {
        ...
        public async Task Update(int id, SectionModel model)
        {
            ...
            string _sql = $"UPDATE section SET content = {b64Content}, name = {model.Name} where  WHERE ROWID = {model.Id}";
        ...
        {
            var tag = "{{" + model.Name.Replace(" ", "_") + "}}";

            var b64Content = !string.IsNullOrEmpty(model.Content) ? model.Content.ToBase64() : string.Empty.ToBase64();
            
            //INSERT INTO section (name, content, tag, modifiable) VALUES ('title', '', '{{title}}', 0);
            string _sql = $"INSERT INTO section (name, content, tag, modifiable,createdAt) VALUES ('{model.Name}', '{model.Content}', '{tag}', 1, {DateTime.Now.Ticks}) Returning RowId";
        ...
        public async Task<SectionResult> List(int count, int offset)
        {
